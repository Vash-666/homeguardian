...
# HomeGuardian Health Check Orchestrator
...
mkdir -p "$DATA_DIR/metrics" "$LOG_DIR" "$DATA_DIR/baseline"
...
process_alerts() {
    ...
    alert_level=$(echo "$json_output" | python3 -c "import sys, json; data=json.load(sys.stdin); print(data.get('alert', {}).get('level', 'INFO'))")
    alert_message=$(echo "$json_output" | python3 -c "import sys, json; data=json.load(sys.stdin); print(data.get('alert', {}).get('message', ''))")
    alert_details=$(echo "$json_output" | python3 -c "import sys, json; data=json.load(sys.stdin); print(data.get('alert', {}).get('details', ''))")
...
check_alert_cooldown() {
    local alert_key=$1
    local cooldown_file="$DATA_DIR/alert_cooldown_${alert_key}.txt"
...
run_health_check() {
    ...
    if [ ${#module_results[@]} -gt 0 ]; then
        ...
        overall_health=$(echo "$health_summary" | python3 -c "import sys, json; data=json.load(sys.stdin); print(data['overall_health'])")
